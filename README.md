...
- **재귀적 패턴 병합 (Recursive Merge)**: 모든 이미지의 공통 기반(Universal Base)을 먼저 추출한 뒤, 차이점만을 `{A|B|C}` 형태로 중첩 압축합니다. 최대 깊이(Max Depth) 설정을 통해 복잡도를 조절할 수 있습니다. 병합을 원치 않으면 병합 트리거 일치율을 의미하는 SIMILARITY 슬라이더를 1.0으로 조정하면 됩니다.
- **강력한 태그 필터링**: 특정 단어 포함/일치 여부, 단어 수 제한, 예외 목록 관리 등을 통해 원치 않는 태그를 사전에 걸러냅니다.
- **실시간 확장 확인**: 생성된 와일드카드 패턴이 실제로 어떻게 확장되는지 미리 확인하고 `.txt` 파일로 내보낼 수 있습니다.

### 2. Batch Crop Module (학습 데이터셋 전처리, 이미지 후처리)
